...
import { Building, Calendar, ExternalLink, Timer } from "lucide-react";
...
function Test({ className, test }: TestPropsType): JSX.Element {
  const hasLink = Boolean(test.url);
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <div className="flex-1">
              <span className="text-gray-500 text-xs uppercase tracking-wide font-medium">
                Отдел:{" "}
                <span
                  className={`font-medium tabular-nums ${test.note_position ? "text-gray-400" : "text-gray-900"}`}
                >
                  {test.position}
                  {test.note_position && (
